...
const SEARCH_TOOL = "https://docs.enclaws.ai/mcp.SearchEnClaws";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.enclaws.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('enclaws docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.enclaws.ai/");
      runtime.log(`Search: ${formatCliCommand('enclaws docs "your query"')}`);
    }
    return;
  }

...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawAgentDir: vi.fn().mockReturnValue("/tmp/enclaws-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/enclaws-agent"),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/enclaws-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/enclaws-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/enclaws-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/enclaws-agent-custom");

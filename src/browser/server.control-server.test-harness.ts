...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/enclaws" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          enclaws: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.ENCLAWS_GATEWAY_PORT;
  process.env.ENCLAWS_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ENCLAWS_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ENCLAWS_GATEWAY_PASSWORD;

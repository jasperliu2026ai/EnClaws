...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "enclaws-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateOpenClawProfile(userDataDir, { color: DEFAULT_ENCLAWS_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".enclaws-profile-decorated"),
      "utf-8",
    );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME);
  });
});

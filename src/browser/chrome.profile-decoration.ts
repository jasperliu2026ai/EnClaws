...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".enclaws-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ENCLAWS_BROWSER_COLOR).toUpperCase();

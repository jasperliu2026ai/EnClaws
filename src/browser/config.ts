...
import {
  ...
  DEFAULT_ENCLAWS_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ENCLAWS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "enclaws" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ENCLAWS_BROWSER_PROFILE_NAME] = {
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for enclaws CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_ENCLAWS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "enclaws";

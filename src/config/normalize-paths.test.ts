  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.enclaws/logs/enclaws.log" },
        hooks: {
          path: "~/.enclaws/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.enclaws/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".enclaws", "logs", "enclaws.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".enclaws", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".enclaws", "telegram.token"),
      );

...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ENCLAWS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*enclaws-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "enclaws";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "enclaws");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "enclaws-gw");
  ...
  records.push(`_enclaws-gw._tcp IN PTR ${instanceLabel}._enclaws-gw._tcp`);
  records.push(`${instanceLabel}._enclaws-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._enclaws-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; enclaws-content-hash: ${contentHash}\n${contentBody}`;
}

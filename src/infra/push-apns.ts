...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ENCLAWS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ENCLAWS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ENCLAWS_APNS_TEAM_ID and ENCLAWS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ENCLAWS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ENCLAWS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ENCLAWS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ENCLAWS_APNS_PRIVATE_KEY_P8 or ENCLAWS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ENCLAWS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    enclaws: createOpenClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };

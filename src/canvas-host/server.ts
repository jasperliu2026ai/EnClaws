...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.enclawsCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.enclawsCanvasA2UIAction &&
        typeof window.enclawsCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.enclawsSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("enclaws:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android EnClaws node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.enclawsSendUserAction === "function"
        ? window.enclawsSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ENCLAWS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ENCLAWS_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>EnClaws Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

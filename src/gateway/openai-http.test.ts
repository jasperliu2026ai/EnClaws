    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "enclaws", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-enclaws-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-enclaws-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "enclaws", messages: [{ role: "user", content: "hi" }] },
          {
            "x-enclaws-agent-id": "beta",
            "x-enclaws-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from EnClaws.");
      }

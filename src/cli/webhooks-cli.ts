...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.enclaws.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + EnClaws hooks")
    ...
    .option("--hook-url <url>", "EnClaws hook URL")
    .option("--hook-token <token>", "EnClaws hook token")

...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ENCLAWS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ENCLAWS_HOME: "/srv/enclaws-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/enclaws-home"), ".enclaws", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".enclaws", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("enclaws-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclaws-workspace-hardlink-"));
    try {

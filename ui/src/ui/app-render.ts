    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""}">
        ...
        ${TAB_GROUPS.map((group) => {
          // Filter tabs by user role — member/viewer cannot see admin-only tabs
        ...
        ${(() => {
          ...
          return html`
            <div class="nav-user-footer" style="padding: 0.75rem; border-top: 1px solid var(--border, #262626); margin-top: auto; font-size: 0.8rem;">
              <div style="display: flex; align-items: center; justify-content: space-between; gap: 0.5rem;">
                ${state.settings.navCollapsed
                  ? html`<button
                    ...
                    >⏻</button>`
                  : html`
                    <span style="color: var(--text-secondary, #a3a3a3); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title=${authState.user.email}>

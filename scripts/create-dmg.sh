...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "EnClaws")
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/enclaws-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/enclaws-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/enclaws-dmg-limits.txt
